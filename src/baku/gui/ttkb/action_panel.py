...
class ActionPanel(tb.Frame):
    def __init__(self, master, main_app):
        ...
        queue_frame.pack(side=LEFT, fill=Y, padx=(0, 10))
        ...
        manage_frame.pack(side=LEFT, fill=Y, padx=(0, 10))
    ...
    def update_stats(self):
        ...
        try:
            ...
            success = sum(1 for item in queue_items if item.status == FileStatus.SUCCESS)
            failed = sum(1 for item in queue_items if item.status == FileStatus.FAILED)
            skipped = sum(1 for item in queue_items if item.status == FileStatus.SKIPPED)
